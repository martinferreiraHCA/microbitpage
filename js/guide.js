...
(function (global) {
  ...
  const STEPS = [
    ...
    { title: '8. Cámara + Teachable Machine', target: null, html: `
      ...
      <p>Agregá el elemento <b>Cámara / IA</b>, elegí "Teachable Machine" en <i>Modelo a usar</i>, pegá la URL e iniciá la cámara. Vas a tener las variables <code>camara_clase</code> y <code>camara_confianza</code>, y el evento <b>cuando la cámara detecta…</b> en los bloques.</p>`, action: { label: 'Agregar cámara', fn: () => Dashboard.add('camara') } },
    { title: '8b. IA con ML - micro:bit', target: '#btn-ml', html: `
      <p>Con <b>IA · ML micro:bit</b> entrenás un modelo de imagen, audio o pose (queda guardado en este navegador). Después volvés al panel: el elemento <b>Cámara / IA</b> ejecuta ese modelo con la cámara, sin Bluetooth ni ventanas aparte.</p>
      <p>Presioná <b>▶ Iniciar cámara</b> dentro del elemento. Cada vez que cambia la clase, el panel le manda al micro:bit <code>clase:NOMBRE,certeza:NN</code> por el cable, y los bloques (<b>cuando la cámara detecta…</b>) pueden mandar otras órdenes. En <b>Herramientas → Cómo conectar ML - micro:bit</b> está el programa del micro:bit y los pasos.</p>
